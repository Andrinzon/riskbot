...
async def manejar_botones(update: Update, context: CallbackContext):
    ...
    if query.data == "precio_btc":
        context.args = ["btc"]
        update.message = query.message  # Redireccionar al comando
...
def main():
    ...
    app.add_handler(MessageHandler(filters.StatusUpdate, manejar_botones))
    app.add_handler(MessageHandler(filters.ALL, manejar_botones))
